    ...
    def process_traffic_lights(self):
        ...
        if(self.pose):
            ...
            for i, light in enumerate(self.lights):
                light_waypoint_index = self.get_closest_waypoint(stop_line_positions[i][0],stop_line_positions[i][1])
                diff = light_waypoint_index - car_position

                if diff >=0 and diff < min_diff:
                    min_diff = diff
                    closest_light = light
                    light_wp = light_waypoint_index


        if closest_light:
            # TODO: use the classifier
